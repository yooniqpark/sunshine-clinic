...
- "Sunshine 피부과" → "Sunshine Dermatology Clinic" (en) / "Sunshine 皮膚科" (ja) / "Sunshine 皮肤科" (zh).
- "원장" → "Director" / "院長" / "院长".
...
export async function POST(req: Request) {
  ...
  try {
    ...
    for (const locale of LOCALES) {
      const system = `You are a professional medical translator for Sunshine Dermatology Clinic. Translate Korean → ${LANG_LABEL[locale]}.\n${RULES}`;
