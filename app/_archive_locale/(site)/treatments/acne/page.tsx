...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const c = getSiteContent(locale).categories.acne;
  return { title: c.label, description: c.description };
}

export default async function AcnePage({ params }: Props) {
  const { locale } = await params;
  const content = getSiteContent(locale);
  const cat = content.categories.acne;
  const tt = content.treatment;
  const common = content.common;
  const concerns = getConcernsByCategory(locale, "acne");
  const devices = getDevicesByCategory(locale, "acne");
