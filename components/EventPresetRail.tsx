...
type FirstVisitEventId = Extract<EventPresetId, "first-visit-2026" | "first-visit-magazine-2026">;
type SwitchableEventId = Extract<EventPresetId, "grand-open-2026"> | FirstVisitEventId;

function tabs(firstVisitPreset: FirstVisitEventId) {
  return [
    { id: "grand-open-2026" as const, index: "01", label: "OPEN EVENT" },
    { id: firstVisitPreset, index: "02", label: "FIRST VISIT" },
  ];
}
...
export function EventPresetTabs({
  ...
  return (
    <nav
      aria-label="이벤트 전환"
      className="grid h-16 shrink-0 grid-cols-2 overflow-hidden rounded-t-[2rem] border-b border-white/15 shadow-lg"
    >
      {tabs(firstVisitPreset).map((tab) => {
        const active = tab.id === activePreset;
        const openEvent = tab.id === "grand-open-2026";
